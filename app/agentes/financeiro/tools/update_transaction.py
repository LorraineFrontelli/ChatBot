...
def _locate_target_ids(
    ...
    if not rows or len(rows) < 1:
        logger.error("No matching transaction found to update")
    ...
    try:
        with get_cursor() as cur:
            ...
            if category_name and not category_id:
                resolved_category_id = resolve_category_id(cur, None, category_name)
            logger.debug("Resolved category id: %s", resolved_category_id)
        ...
        logger.exception("Exception raised while updating transaction")
